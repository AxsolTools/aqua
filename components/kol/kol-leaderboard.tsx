...
export function KOLLeaderboard({ onSelectKOL, selectedKOL }: Props) {
  ...
  const getRankDisplay = (index: number, globalIndex: number) => {
    if (globalIndex === 0)
      return (
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600 flex items-center justify-center text-[var(--ocean-deep)] font-black">
          1
        </div>
      )
    if (globalIndex === 1)
      return (
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-neutral-300 to-neutral-500 flex items-center justify-center text-[var(--ocean-deep)] font-black">
          2
        </div>
      )
    if (globalIndex === 2)
    ...
    return (
      <div className="w-8 h-8 rounded-full bg-[var(--bg-secondary)] flex items-center justify-center text-[var(--text-muted)] font-bold">
        {globalIndex + 1}
      </div>
    )
  }
      ...
      <div className="p-4 border-b border-[var(--border-subtle)]">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-[var(--text-primary)]">KOL LEADERBOARD</h2>
        ...
        <div className="flex gap-2 mb-3">
          ...
          <select
            ...
            className="px-3 py-2 bg-[var(--bg-secondary)] border border-[var(--border-subtle)] rounded-lg text-sm text-[var(--text-primary)] focus:outline-none focus:border-[var(--aqua-primary)]/50"
          >
        ...
        <div className="flex items-center gap-3 mb-3">
          <label className="flex items-center gap-2 text-xs text-[var(--text-muted)] cursor-pointer">
            <input
              ...
              className="rounded bg-[var(--bg-secondary)] border-[var(--border-default)]"
            />
            Verified Only
          </label>
          <label className="flex items-center gap-2 text-xs text-[var(--text-muted)] cursor-pointer">
        ...
        {paginatedKols.map((kol, index) => {
          ...
          return (
            <div
              ...
              className={cn(
                "p-4 border-b border-[var(--border-subtle)] cursor-pointer transition-all hover:bg-[var(--bg-secondary)]",
                selectedKOL?.id === kol.id && "bg-[var(--aqua-primary)]/5 border-l-2 border-l-[var(--aqua-primary)]",
                kol.isWashTrader && "bg-[var(--red)]/5"
              )}
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-[var(--text-primary)] truncate">{kol.name}</span>
                    {getTierIcon(kol.tier)}
                    {kol.isWashTrader && (
                      <span className="text-[10px] bg-[var(--red)]/20 text-[var(--red)] px-1.5 py-0.5 rounded font-medium">
                        WASH TRADER
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <a
                      ...
                      className="text-xs text-[var(--text-muted)] hover:text-[#1d9bf0]"
                    >
                      @{kol.twitter}
                    </a>
                    <span className="text-[var(--text-dim)]">•</span>
                    <span className="text-xs text-[var(--text-dim)]">{kol.tradingStyle}</span>
                  </div>
                  <div className="flex items-center gap-3 mt-2">
                    <div className="flex items-center gap-1">
                      <Target className="w-3 h-3 text-[var(--text-muted)]" />
                      <span className="text-xs text-[var(--text-muted)]">{kol.winRate.toFixed(1)}%</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Zap className="w-3 h-3 text-[var(--text-muted)]" />
                      <span className="text-xs text-[var(--text-muted)]">{kol.totalTrades.toLocaleString()}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <Users className="w-3 h-3 text-[var(--text-muted)]" />
                      <span className="text-xs text-[var(--text-muted)]">{(kol.followers / 1000).toFixed(0)}K</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="text-right">
                  ...
                  <div
                    className={cn(
                      ...
                      kol.pnl7d >= 0 ? "text-[var(--green)]/70" : "text-[var(--red)]/70"
                    )}
                  >
                  ...
                  <div className={cn(
                    "text-xs",
                    kol.roi30d >= 0 ? "text-cyan-400" : "text-[var(--red)]"
                  )}>
                  ...
                  <div className="flex items-center justify-end gap-1 mt-2">
                    <button
                      ...
                      className={cn(
                        "p-1.5 rounded hover:bg-[var(--bg-elevated)] transition-colors",
                    ...
                    <button
                      ...
                      className="p-1.5 rounded hover:bg-[var(--bg-elevated)] transition-colors text-[var(--text-dim)] hover:text-[var(--text-primary)]"
                      title="Copy wallet"
                    >
        ...
        <div className="p-4 border-t border-[var(--border-subtle)] flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={page === 1}
              className="px-3 py-1.5 bg-[var(--bg-secondary)] border border-[var(--border-subtle)] rounded text-sm text-[var(--text-primary)] disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[var(--bg-elevated)]"
            >
              Prev
            </button>
